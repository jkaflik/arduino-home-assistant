...
#define HACOVER_CALLBACK(name) void (*name)(HACover *cover, CoverCommand cmd)
#define HACOVER_POSITION_CALLBACK(name) void (*name)(HACover *cover, int16_t position)
...
public:
    ...
    /**
     * Registers callback that will be called each time the position change from HA is received.
     * Please note that it's not possible to register multiple callbacks for the same covers.
     *
     * @param callback
     */
    inline void onPositionChange(HACOVER_POSITION_CALLBACK(callback))
...
protected:
    ...
    void handlePosition(int16_t position);

    HACOVER_CALLBACK(_commandCallback);
    HACOVER_POSITION_CALLBACK(_positionCallback);
